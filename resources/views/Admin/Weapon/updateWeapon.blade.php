            ...
            <div class="content_manager_list_user">
                <form action="{{ route('weapon.update', $weapon->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="detail_list_users">
                        @if (isset($weapon) && is_object($weapon))
                            <div class="half_infor_list_users">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" value="{{ old('name', $weapon->name) }}" class="form-control"
                                        placeholder="{{ $weapon->name }}" name="name" id="" />
                                    @if ($errors->has('name'))
                                        ...
                                        @if (isset($categories) && is_object($categories))
                                            @foreach ($categories as $category)
                                                <option value="{{ $category->id }}"
                                                    {{ $weapon->category_id == $category->id ? 'selected' : '' }}>
                                                    {{ $category->game_type }}
                                                </option>
                                            @endforeach
                                        @endif
                    ...
                    <div class="go_back">
                        <a href="{{ route('weapon.index') }}" class="btn btn-light"><i
